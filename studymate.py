...
st.markdown("""
    ...
    .user-message { background-color: #d9caff !important; border-left: 4px solid #7b42f5 !important; color: #000000 !important; }
    .assistant-message { background-color: #ffffff; border-left: 4px solid #4CAF50; color: #000000 !important; }
    .header-container {
...
for key in ['messages', 'pdf_texts', 'embeddings', 'faiss_index', 'model', 'tokenizer', 'embedding_model', 'voice_processed']:
    if key not in st.session_state:
        st.session_state[key] = [] if key in ['messages', 'pdf_texts'] else None
...
def load_models(hf_token: str):
    try:
        ...
        st.info("💡 Ensure your Hugging Face token has access to IBM Granite model.")
        return False
...
with st.sidebar:
    ...
    hf_token = st.text_input("🔑 Hugging Face Access Token", type="password", help="Enter your Hugging Face token to use IBM Granite 3.2 2B Instruct model")
    ...
    uploaded_files = st.file_uploader("Upload PDF files", type=['pdf'], accept_multiple_files=True, help="Upload your textbooks, lecture notes, or research papers")
    ...
    if st.session_state.pdf_texts:
        ...
        with col1:
            st.markdown(f"<div class='stats-box'><h3 style='color: #667eea; margin:0;'>{len(st.session_state.pdf_texts)}</h3><p style='margin:0; color: #666;'>Text Chunks</p></div>", unsafe_allow_html=True)
        with col2:
            st.markdown(f"<div class='stats-box'><h3 style='color: #764ba2; margin:0;'>{len(st.session_state.messages)}</h3><p style='margin:0; color: #666;'>Messages</p></div>", unsafe_allow_html=True)
...
with col1:
    ...
    for message in st.session_state.messages:
        if message["role"] == "user":
            ...
            st.markdown(f"<div class='chat-message assistant-message'><strong>🤖 StudyMate:</strong><br>{message['content']}</div>", unsafe_allow_html=True)
...
# ---------------------- TEXT INPUT ---------------------- #
if st.session_state.model and st.session_state.pdf_texts:
...
# Footer
st.markdown("---")
st.markdown("<div style='text-align: center; color: #666; padding: 1rem;'><p>🎓 StudyMate - Empowering students with AI-powered learning assistance</p><p style='font-size: 0.9rem;'>Powered by IBM Granite 3.2 2B Instruct</p></div>", unsafe_allow_html=True)
